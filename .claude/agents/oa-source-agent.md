...
- Don't append a "Generated by/with Claude Code" signature to PR descriptions or issue bodies/descriptions in this repo — GitHub's own commit/PR authorship metadata already covers it, and it's just noise on content other contributors read. (This is distinct from the attribution footer your harness may still require on GitHub *comments* specifically — that one isn't a per-repo style choice and stays.)
