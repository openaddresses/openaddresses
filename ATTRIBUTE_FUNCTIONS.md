...
**Important:** any step in `functions` after the first must reference the working value with an `oa:` prefix on the `variable` name (`oa:street_wip` above), not the bare `variable` name (`street_wip`). Each step's result is always written to the `oa:`-prefixed field, but it is only copied to the unprefixed field when the result is non-empty. If a step's result happens to be empty for some rows (e.g. the source field is blank, or a pattern doesn't match), the unprefixed field may be missing or stale on those rows, so a later step that reads the bare `variable` name will silently fail or produce the wrong output for just those rows. Referencing the `oa:`-prefixed name avoids this. The same applies if `variable` is omitted: `chain` then accumulates its result directly in the output field itself, so later steps must reference it as `oa:` plus the output field's own name (e.g. `oa:street`).
